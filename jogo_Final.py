from Transforma_Base_de_Questões import transforma_base
from sorteie_uma_questao import sorteia_questao
from soteia_questao_inedita import sorteia_questao_inedita
from questao_para_texto import questao_para_texto
from gera_ajuda import gera_ajuda
from Valida_Uma_Questão import valida_questao
from Valida_Lista_de_Questões import valida_questoes

nome = str(input('Diga-me seu nome!'))
print("Olá, bom dia e bem vindo ao jogo! ")
print('Ok {0}, você tem direito a pular 3 vezes e 2 ajudas!'.format(nome))
print ('Você deve responder em formato "A", "B", "C", podendo também pedir "ajuda", pular a questão por meio de "pula" e assim que quiser parar, é só digitar "pare"!')

comece = input('Digite "comece" para o jogo comecar!')

if comece == "comece":
    print('Agora sim, chega de enrolação, o jogo irá começar!!!')
...
sorteadas = []
numero = 1
questoes_para_uso = transforma_base

print('Iremos iniciar com questões mais fáceis, passando para o nivel médio e depois as difíceis.')
comecar = input('Aperte ENTER para continuar...')

ganha = [0, 1000, 5000, 10000, 30000, 50000, 100000, 300000, 500000, 1000000]
a = 1
b = 0
c = 0

gerar_nova_questao = True
while True:

    if gerar_nova_questao:

        questao = sorteia_questao_inedita
        texto_utilizavel = questao_para_texto
        print(texto_utilizavel)
        gerar_nova_questao = False
    
    resp = input('Qual vai ser a sua resposta? Será essa a certa??? ')
    ...
    if resp == 'pula':
        ...
        if b <= 2:
            ...
            print('Você já pulou o máximo de questões possíveis!')
            resp = input('Qual vai ser a sua resposta? Será essa a certa??? ')
    ...
    if resp == 'ajuda':
        ...
        if c <= 1:
            ...
            print('Você já pediu o máximo de ajudas possíveis!')
            resp = input('Qual vai ser a sua resposta? Será essa a certa??? ')

    if resp == questao['correta']:

        print('Parabéns, acertou! Assim, seu prêmio vale{0:.2f}!!!'.format(ganha[a]))

        if ganha[a] == 1000000:

            print('COMO ASSIM! Você acabou de ganhar o jogo, então, seu prêmio será de 1000000!!!')
            break
        ...
        if numero == 3:
            ...
            nivel = 'medio'
            print('Se prepare, as questões ficaram mais pesadas, e se tornaram de nivel médio!')
        
        elif numero == 7:
            nivel = 'dificil'
            print('Se prepare, as questões ficaram mais pesadas, e se tornaram de nivel difícil!')

        gerar_nova_questao = True  

    elif resp not in ["A", "B", "C", "D", 'ajuda', 'pula', 'parar']:

        print('A sua opção é inválida!')
        print('Você só pode responder "A", "B", "C", "D", "ajuda", "pula" e "parar"!')
        continue

    elif resp != questao['correta'] and resp in ["A", "B", "C", "D"]:

        print('Deu ruim! Perdeu tudo!!!')
        break

    if resp == 'parar':

        print('Boa jogada, você saiu com um total de {0:.2f}! Está bom, mas da pra melhorar...'.format(ganha[a - 1]))
        break